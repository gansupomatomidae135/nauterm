...
class Window : public NativeObjectProvider, public std::enable_shared_from_this<Window> {
 public:
  ...
  /**
   * @brief Sets the position of window control buttons relative to the window.
   *
   * @param x The x offset from the left edge of the content view (in points)
   * @param y The y offset from the top of the content view (in points)
   *
   * On macOS, this repositions the traffic light buttons (close, minimize, zoom).
   * The coordinate system has the origin at the top-left of the content area.
   *
   * @note Platform availability:
   * - macOS: ✅ Fully supported - Moves the traffic light buttons
   * - Windows: ❌ Not applicable - Uses system-determined position
   * - Linux: ❌ Not applicable - Uses system-determined position
   */
  void SetWindowControlButtonsPosition(double x, double y);

  /**
   * @brief Gets the current position of window control buttons.
   *
   * @return Point The position of the window control buttons
   *
   * @note Platform availability:
   * - macOS: ✅ Fully supported
   * - Windows: ❌ Not implemented - Returns {0, 0}
   * - Linux: ❌ Not implemented - Returns {0, 0}
   */
  Point GetWindowControlButtonsPosition() const;
