...
FFI_PLUGIN_EXPORT
void native_window_set_window_control_buttons_position(native_window_t window, double x, double y) {
  if (!window)
    return;
  auto* win = static_cast<nativeapi::Window*>(window);
  win->SetWindowControlButtonsPosition(x, y);
}

FFI_PLUGIN_EXPORT
native_point_t native_window_get_window_control_buttons_position(native_window_t window) {
  native_point_t result = {0.0, 0.0};
  if (!window)
    return result;
  auto* win = static_cast<nativeapi::Window*>(window);
  auto pos = win->GetWindowControlButtonsPosition();
  result.x = pos.x;
  result.y = pos.y;
  return result;
}
